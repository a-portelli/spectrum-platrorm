...
public class readNodeB : MonoBehaviour
{
    //Declare variables
    ...
    private void Awake() 
    {
        ...
        {
            //Create origin root 
            string oPath = "Assets/nodeB/";
            string oLinkRoot = "NodeB_";
            int x = b;
    ...
    void Start()
    {
       ...
       // Create and position the sphere. Reduce the size.
        ...
        for (int i = 2; i < 713; i++)
            {
                //Create root 
                string path = "Assets/nodeB/";
                string linkRoot = "NodeB_";
                int x = i;
    ...
    void Update()
    {
        ...
        if (Vector3.Distance(transform.position, target.position) < 0.001f && j < 713) 
        {
            if(j == 0 || j==1){
                ...
                latDisplay = latArr[j];
                lonDisplay = lonArr[j];
                latText = latArr[j].ToString();
                lonText = lonArr[j].ToString();
    ...
    // using (var writer = new StreamWriter("Assets/latVals.txt")){
    ...
    //         else
    //         {
    //             writer.WriteLine(latArr[zz].ToString());
    //         }
    //     }  
    // }
