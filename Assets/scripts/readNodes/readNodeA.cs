...
public class readNodeA : MonoBehaviour
{
    //Declare variables
    ...
    private void Awake() 
    {
        ...
        {
            //Create origin root 
            string oPath = "Assets/nodeA/";
            string oLinkRoot = "NodeA_";
            int x = b;
        ...
        if(Pause.GameIsPaused == false){
            ...
            if (Vector3.Distance(transform.position, target.position) < 0.001f && j < 713) 
            {
                ...
                else
                {
                    ...
                    lonDisplay = lonArr[j];
                    j++;
                    if(j % 10 == 0) //every 10 frames, drop a trail box
                    {
